...
file_to_process2 = "CLAIM FILE.txt"
file_to_process3 = "CLAIM FILE3.txt"
file_to_process4 = "HJ2537.004"
file_to_process5 = "HJ2537.005"
...
def sendrequest(xmlPayload: str) -> Tuple[int, str]:
    ...
    global method, claimfile, claimfile2, claimfile3, claimfile4, claimfile5, claimfile6
    if method in ['upload', 'update']:
        # Determine which files to include based on method
        if method == 'upload':
            files_to_send = [claimfile,]
        elif method == 'update':
            files_to_send = [claimfile,]
        ...
        for filepath in files_to_send:
            try:
                with open(filepath, 'rb') as f:
                    file_contents[filepath] = f.read().decode('utf-8')
        ...
        # Add each file as a separate MIME part
        for filepath, fileContent in file_contents.items():
            mimeMessage += f"--{boundary}\r\n"
            mimeMessage += "Content-Type: text/plain; charset=us-ascii\r\n"
            mimeMessage += "Content-Transfer-Encoding: 7bit\r\n"
            mimeMessage += f"Content-ID: <{filepath}>\r\n"
            mimeMessage += f"Content-Disposition: attachment; name=\"{filepath}\"\r\n\r\n"
